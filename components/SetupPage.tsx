...
const labels = {
  en: {
    ...
    setupIntro: 'Configure spin lookback depth for the 5 prediction criteria to optimize pattern analysis and number convergence.',
    ...
    defaultNotice: 'Default depth is 10 spins. Final Matrix uses full-session transitions (≥2 hits) and Sectors use destination transitions from the last spin. Top 3 Numbers synthesizes all criteria (Final Matrix, Pockets, Sectors, Colour, Series, Dozens/Cols, and Patterns) to predict the 3 highest chance numbers.',
  ...
  zh: {
    ...
    setupIntro: '配置5大预测标准的历史旋转分析深度，精准优化模式识别与数字概率收敛。',
    colorDepth: '🎨 颜色预测深度',
    seriesDepth: '🧭 分区预测深度',
    pocketsDepth: '📏 口袋步距深度',
    ...
    spins: '转',
    resetAll: '重置全为10转',
    defaultNotice: '默认深度为10转。尾数矩阵基于整场转移（≥2次命中），扇区基于上一转后的流向转移。前3推荐号码综合全标准数据（尾数矩阵、口袋距离、扇区、颜色、分区、打几十/打列及走势规律）深度分析并预测胜率最高的3个号码。',
  ...
  ja: {
    ...
    setupIntro: '5つの予測基準のスピン分析深度を設定し、パターン認識と確率収束を最適化します。',
    colorDepth: '🎨 カラー予測深度',
    ...
    defaultNotice: 'デフォルト深度は各10スピンです。尾数マトリクスは全履歴遷移（≥2回ヒット）、扇区は前回スピンからの推移に基づきます。トップ3数字は全基準データ（尾数マトリクス、ポケット、扇区、カラー、シリーズ、ダズン/カラム、パターン）を統合分析し最高確率の3数字を予測します。',
  ...
  es: {
    ...
    setupIntro: 'Configure la profundidad de giros de los 5 criterios de predicción para optimizar la convergencia de probabilidad.',
  ...
  ko: {
    ...
    setupIntro: '5가지 예측 기준별 스핀 탐색 깊이를 설정하여 최적의 패턴 인식과 확률 수렴을 구성합니다.',
    colorDepth: '🎨 색상 예측 깊이',
    ...
    defaultNotice: '기본 탐색 깊이는 각 10스핀입니다. 끝수 매트릭스는 세션 전체 전이(≥2회 적중)를 사용하며, 섹터는 최근 스핀 후속 전이를 기반으로 합니다. 톱 3 번호는 모든 기준(끝수 매트릭스, 포켓, 섹터, 색상, 구역, 더즌/컬럼 및 패턴)을 통합 분석하여 가장 승률 높은 3개 번호를 예측합니다.',
  ...
  vi: {
    ...
    setupIntro: 'Cấu hình độ sâu vòng quay cho 5 tiêu chí dự đoán để tối ưu hóa nhận diện mẫu và hội tụ xác suất số.',
    ...
    defaultNotice: 'Độ sâu mặc định là 10 vòng quay. Ma Trận Số Cuối tự động dùng chuyển tiếp cả phiên (≥2 lần trúng) và Ô Bánh Xe dùng bước chuyển từ số trước. 3 Số Hàng Đầu kết hợp toàn bộ dữ liệu tiêu chí (Ma trận số cuối, Ô khoảng cách, Ô bánh xe, Màu sắc, Phân vùng, Hàng tá/Cột và Mẫu chuỗi) để phân tích và dự đoán 3 số có cơ hội cao nhất.',
